    ...
    public partial class Line2WholeTrk : Form
    {
        ...
        private void AsyncUpdateUIData()
        {
            ...
            while (!IsDisposed)
            {
                ...
                sw.Stop();
                Console.WriteLine("Line2 update Elapsed:" + sw.Elapsed.TotalMilliseconds);
        ...
        private void InitButton()
        {
            ...
            SetupButton(btnZ02Fin, Acltable);
        }
            ...
            if (moddt != null)
            {
                try
                {
                    ...
                    if (moddt.Rows[0]["STEEL_NM"].ToString() == "SAE51B35")
                    {
                        if (length <= 6.1)
                        {
                            label8.Text = "현재 POC(" + poc_no_nm + ")는 단척제품입니다 참고바랍니다.";
                            label8.Visible = true;
                        }
                        else
                        {
                        ...
                        //label8.Text = "현재 POC(" + poc_no_nm + ") ND(SAE51B35) 강종은 난단척 주의 강종입니다";
                        //label8.Visible = true;
                    }
                    else
                    {
        ...
        private void btnZ02Fin_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(" Z02 이후 트레킹이 종료됩니다 진행하시겠습니까? ", Text, MessageBoxButtons.YesNo) == DialogResult.No)
            {
                return;
            }

            if (CallZ02FinSP(line_gp, ck.UserID))
            {
                btnDisplay_Click(null, null);
            }
        }
        ...
        /// <returns></returns>
        private bool CallZ02FinSP(string _line_gp, string _userID)
        {
