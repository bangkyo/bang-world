...
namespace SystemControlClassLibrary
{
    ...
    public partial class CrtInRsltPopUP : Form
    {
        ...
        public CrtInRsltPopUP(string _line_gp, string _mill_no)
        {
            ...
            txtMillNo.Text = _mill_no;

        }

        public CrtInRsltPopUP(string _line_gp, string _mill_no, bool _CanModify) : this(_line_gp, _mill_no)
        {
            this.CanModify = _CanModify;

            btnReg.Enabled = CanModify;
        }

        public void SetMillNo(string _mill_no)
        {
            txtMillNo.Text = _mill_no;

            if (!string.IsNullOrEmpty(txtMillNo.Text))
            {
                txtMillNo.Focus();
                SendKeys.Send("{Enter}");
        ...
        private void SaveReg()
        {
            ...
            if (MessageBox.Show("투입하시겠습니까?", Text, MessageBoxButtons.YesNo) == DialogResult.No)
            {
                return;
            }
            #endregion     
