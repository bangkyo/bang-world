...
namespace SystemControlClassLibrary.information
{
    public partial class ScreenMgmt : Form
    {
        ...
        private void InitGrd_Main()
        {
            ...
            var crCellRange = grdMain.GetCellRange(0, grdMain.Cols["BIZ_GP"].Index);
            crCellRange.Style = grdMain.Styles["ModifyStyle"];

            crCellRange = grdMain.GetCellRange(0, grdMain.Cols["SCR_NM"].Index, 0, grdMain.Cols["USE_YN"].Index);
            crCellRange.Style = grdMain.Styles["ModifyStyle"];
            ...
            grdMain.Cols["SCR_ID"].Width = 250;
            grdMain.Cols["SCR_NM"].Width = 360;
            grdMain.Cols["PAGE_ID"].Width = 300;
            ...
            grdMain.Cols["BIZ_GP"].TextAlign = C1.Win.C1FlexGrid.TextAlignEnum.CenterCenter;
            grdMain.Cols["SCR_ID"].TextAlign = C1.Win.C1FlexGrid.TextAlignEnum.CenterCenter;
            grdMain.Cols["SCR_NM"].TextAlign = C1.Win.C1FlexGrid.TextAlignEnum.LeftCenter;
           grdMain.Cols["PAGE_ID"].TextAlign = C1.Win.C1FlexGrid.TextAlignEnum.LeftCenter;
        ...
        private bool SetComboBox1()
        {
            try
            {
                cboBIZ_GP.Items.Clear();
                DataTable dt = cd.Find_BIZ_GP();
                Work_cd = new ListDictionary();
                ...
                {
                    list1.Add(dt.Rows[row].ItemArray[0].ToString());
                    Work_cd.Add(dt.Rows[row].ItemArray[0].ToString(), dt.Rows[row].ItemArray[0].ToString());
                }
                foreach (var item in list1)
                {
                    cboBIZ_GP.Items.Add(item);

                }
                //첫번째 아이템 선택
                cboBIZ_GP.SelectedIndex = 0;
        ...
        private void cboBIZ_GP_SelectedIndexChanged(object sender, EventArgs e)
        {
            string line_gp_nm = string.Empty;
            if (cboBIZ_GP.SelectedIndex >= 0)
            {
                cboBiz_nm = cboBIZ_GP.SelectedItem as string;
            }
        ...
        private void btnRowAdd_Click(object sender, EventArgs e)
        {
            ...
            grdMain.SetData(grdMain.Rows.Count - 1, "BIZ_GP", first);
            ...
            try
            {
                ...
                for (row = 1; row < grdMain.Rows.Count; row++)
                {
                    ...
                    if (grdMain.GetData(row, grdMain.Cols.Count - 1).ToString() == "추가")
                    {
                        ...
                        sql1 += string.Format("         '{0}' ", grdMain.GetData(row, "BIZ_GP"));             /* BIZ_GP ");*/
                        sql1 += string.Format("        ,'{0}' ", grdMain.GetData(row, "SCR_ID"));                  /* SCR_ID ");*/
                        sql1 += string.Format("        ,'{0}' ", grdMain.GetData(row, "SCR_NM"));              /* SCR_NM ");*/
                        sql1 += string.Format("        ,'{0}' ", grdMain.GetData(row, "PAGE_ID"));             /* PAGE_ID ");*/
                        sql1 += string.Format("        ,'{0}' ", vf.StringToString(grdMain.GetData(row, "USE_YN").ToString()));             /* USE_YN ");*/
                    ...
                    else if (grdMain.GetData(row, grdMain.Cols.Count - 1).ToString() == "수정")
                    {
                        ...
                        sql1 += string.Format("        BIZ_GP = '{0}' ", grdMain.GetData(row, "BIZ_GP"));
                        sql1 += string.Format("       ,SCR_NM = '{0}' ", grdMain.GetData(row, "SCR_NM"));
                        sql1 += string.Format("       ,PAGE_ID = '{0}' ", grdMain.GetData(row, "PAGE_ID"));
                        ...
                        sql1 += string.Format(" WHERE SCR_ID = '{0}' ", grdMain.GetData(row, "SCR_ID"));
                    ...
                    {
                        sql1 = string.Format("DELETE FROM TB_CM_SCR WHERE SCR_ID = '{0}'", grdMain.GetData(row, 2));
        ...
        private void SetDataBinding()
        {
            ...
            sql1 += string.Format(" from TB_CM_SCR ");
            sql1 += string.Format(" where BIZ_GP LIKE '{0}' || '%' ", cboBiz_nm);
            sql1 += string.Format(" AND SCR_ID LIKE  '%{0}%' || '%' ", txtSCR_ID); //:SCR_ID
            sql1 += string.Format(" AND SCR_NM LIKE '%{0}%' || '%' ", txtSCR_NM);//:SCR_NM 
            sql1 += string.Format(" ORDER BY BIZ_GP, SCR_ID ASC ) X ");
