...
{
    public partial class CrtInOrdYard : Form
    {
        ...
        public CrtInOrdYard(string titleNm, string scrAuth, string factCode, string ownerNm)
        {
        ...
        private void CrtInOrdYard_Load(object sender, EventArgs e)
        {
            InitControl();
        ...
        {
            cd.SetCombo(cbYard, "LG", "", false, ck.Line_gp);
        }


        private void SetComboBox2()
        {
            cd.SetComboYard(cbLoc, "LY", yard, true, ck.Line_gp);
        }
        ...
        {
            yard = ((ComLib.DictionaryList)cbYard.SelectedItem).fnValue;
            SetComboBox2();
        }

        private void cbLoc_Type_SelectedIndexChanged(object sender, EventArgs e)
        {
            loc = ((ComLib.DictionaryList)cbLoc.SelectedItem).fnValue;
        }
        ...
        private void InitGrd_grdMain()
        {
            ...
            grdMain.Cols["LOC"].TextAlign = cs.ITEM_SIZE_TextAlign;
        ...
        private void SetDataBinding()
        {
            ...
            try
            {
                ...
                sql1 += string.Format("SELECT ROWNUM AS L_NO, LOC, LOCA, HEAT, ITEM, ITEM_SIZE, STEEL, ");
                sql1 += string.Format("       STEEL_NAME, LENGTH, BONSU, NET_WEIGHT, ");
                sql1 += string.Format("       ROM_DATE_T,POC_NO ");
                sql1 += string.Format("FROM   ( ");
                sql1 += string.Format("SELECT A.LOCATION AS LOCA, A.HEAT, A.ITEM, A.ITEM_SIZE AS SIZE_NAME, A.STEEL, ");
                sql1 += string.Format("       (SELECT CD_NM FROM TB_CM_COM_CD WHERE CATEGORY = 'LY' AND CD_ID = A.LOCATION) AS LOC, ");
                ...
                sql1 += string.Format("       LOCATION, A.ITEM_SIZE, STEEL_USE, MAX(ROM_DATE) ROM_DATE_T, ");
                ...
                sql1 += string.Format("       WHERE STATUS = 'WP10' ");
                sql1 += string.Format("       AND A.ITEM IN ('OB','SB','RB') ");
                sql1 += string.Format("       AND A.YARD LIKE '%' || :P_YARD || '%' ");
                sql1 += string.Format("       AND A.LOCATION LIKE '%' || :P_LOC || '%' ");
                ...
                sql1 += string.Format("         GROUP BY A.LOCATION, A.HEAT, A.ITEM, A.ITEM_SIZE, A.STEEL, A.LENGTH, STEEL_USE ");
                sql1 += string.Format("         ORDER BY A.LOCATION, A.HEAT, A.ITEM, A.ITEM_SIZE, A.STEEL, A.LENGTH, STEEL_USE ");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string spName = "YARD_UPDATE";  // 프로시저명
            try
            {
                oConn.Open();
        ...
        private void SelectedGrdMain(C1FlexGrid grd, int selected_Row)
        {
            ...
            // subgrd data search
            string _LOCATION = grd.GetData(selected_Row, "LOCA").ToString();
            ...
            //string _yard = grd.GetData(selected_Row, "STEEL").ToString();
            string _length = grd.GetData(selected_Row, "LENGTH").ToString();
            //string 

            InitGridData(false);

            SetDataBindingGrdSub( _item_size, _LOCATION, _HEAT, _length);
        }

        private void SetDataBindingGrdSub(string _item_size, string _LOCATION, string _HEAT, string _length)
        {
            try
            {
                string sql1 = string.Empty;
                ...
                sql1 += string.Format("AND STATUS = 'WP10' ");
                sql1 += string.Format("AND    YARD LIKE '%' || :P_YARD || '%' ");
                sql1 += string.Format("AND    LOCATION LIKE '%' || :P_LOCATION || '%' ");
                ...
                parm[1] = ":P_YARD|" + yard;
                parm[2] = ":P_HEAT|" + _HEAT;
                parm[3] = ":P_LOCATION|" + _LOCATION;
                parm[4] = ":P_LENGTH|" + _length;
       ...
       ArrayList _al = new ArrayList();
        ...
        private void btnPOCFin_Click(object sender, EventArgs e)
        {
            ...
            if (isChecked)
            {
                if (MessageBox.Show("입고취소하시겠습니까?", Text, MessageBoxButtons.YesNo) == DialogResult.No)
                {
                    return;
                }
            ...
            OracleTransaction transaction = null;
            //string spName = "SP_CR_INPUT_ORD_UPD";  // 프로시저명
            string spName = "YARD_UPDATE_BACK";  // 프로시저명
            try
            {
                oConn.Open();
                cmd.Connection = oConn;
