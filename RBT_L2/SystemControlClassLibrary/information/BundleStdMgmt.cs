...
{
    public partial class BundleStdMgmt : Form
    {
        ...
        public BundleStdMgmt(string titleNm, string scrAuth, string factCode, string ownerNm)
        {
            ck.StrKey1 = "";
            ck.StrKey2 = "";
        ...
        private void BundleStdMgmt_Load(object sender, EventArgs e)
        {
            ...
            for (checkrow = 1; checkrow < grdMain.Rows.Count; checkrow++)
            {
                ...
                if (gubun_value == "추가" || gubun_value == "수정")
                {
                    ...
                    if (gubun_value == "추가")
                    {
                        ...
                        check_table_NM1 = "TB_BUNDLE_PIECESTD";
                        ...
                        check_table_NM2 = "TB_BUNDLE_PIECESTD";
                        ...
                        check_table_NM3 = "TB_BUNDLE_PIECESTD";
            ...
            try
            {
                ...
                for (row = 1; row < grdMain.Rows.Count; row++)
                {
                    ...
                    if (grdMain.GetData(row, 0).ToString() == "추가")
                    {
                        ...
                        sql1 = string.Empty;
                        sql1 += string.Format("INSERT INTO TB_BUNDLE_PIECESTD ");
                        sql1 += string.Format("            ( ");
                    ...
                    {
                        sql1 = string.Empty;
                        sql1 += string.Format("UPDATE TB_BUNDLE_PIECESTD ");
                    ...
                    {
                        sql1 = string.Format("DELETE FROM TB_BUNDLE_PIECESTD WHERE ITEM = '{0}' AND ITEM_SIZE = '{1}' AND LENGTH ='{2}' ", grdMain.GetData(row, "ITEM"), grdMain.GetData(row, "ITEM_SIZE"), grdMain.GetData(row, "LENGTH") );
                        cmd.CommandText = sql1;
                        cmd.ExecuteNonQuery();
                        DelCnt++;
                    }
        ...
        private void SetDataBinding()
        {
            ...
            sql1 += string.Format("                    NO3_BND_POINT, ");
            sql1 += string.Format("                    REMARKS ");
            sql1 += string.Format("                    from TB_BUNDLE_PIECESTD  ");
            sql1 += string.Format("                    WHERE ITEM_SIZE LIKE '%' || '{0}' || '%' ", txtitem);
            sql1 += string.Format("                    ORDER BY ITEM_SIZE, LENGTH ASC   ");
            sql1 += string.Format("                  ) X  ");
