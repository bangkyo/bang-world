CREATE PROCEDURE [dbo].[SP_IF_GRD_STR]
 @P_IF_SEQ    NUMERIC(12)
,@P_TC_CD     VARCHAR(4)  = 'F040'
...
------//프로시져 내부 변수정의//------
DECLARE @ERR_PGM        VARCHAR(50)   = 'SP_IF_GRD_STR';
...
DECLARE @BEF_ZONE_CD    VARCHAR(3) = 'Z70';
        ...
        BEGIN TRAN
            ...
            BEGIN
                ...
                SET     ZONE_CD           = @ZONE_CD
                       ...
                       ,GRD_START_DDTT    = CONVERT(DATETIME,@REG_DDTT)
                ...
                SET     STOP_END_DDTT    = GETDATE();
                IF  @@ROWCOUNT = 0
                    BEGIN
                        INSERT INTO TB_GRD_RBGW_MGMT ( STOP_START_DDTT, STOP_END_DDTT )
                               VALUES ( GETDATE(), GETDATE());
                    END;
                --그라인딩 종료후 5분 지연인 경우 정지 시간 등록 CONVERT(DATETIME, @CUR_DATE + ' 08:00:00')
                IF EXISTS ( SELECT STOP_END_DDTT FROM   TB_GRD_RBGW_MGMT 
                            WHERE DATEDIFF(minute, STOP_START_DDTT, STOP_END_DDTT) > 5 )
                    BEGIN
                        SELECT @START_DDTT  = STOP_START_DDTT
                              ,@END_DDTT    = STOP_END_DDTT
                        FROM   TB_GRD_RBGW_MGMT;
                        ----상주 근무이므로 08시 이후만 정지시간임
                        IF  @START_DDTT  < CONVERT(DATETIME, @CUR_DATE + ' 08:00:00')
                            SET  @START_DDTT  = CONVERT(DATETIME, @CUR_DATE + ' 08:00:00');
                        IF  DATEDIFF(minute, @START_DDTT, @END_DDTT) > 5    
                            BEGIN
                                SET  @STOP_RSN = '1';
