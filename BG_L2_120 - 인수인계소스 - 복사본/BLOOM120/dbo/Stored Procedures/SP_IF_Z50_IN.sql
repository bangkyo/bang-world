CREATE PROCEDURE [dbo].[SP_IF_Z50_IN]
 @P_IF_SEQ    NUMERIC(12)
,@P_TC_CD     VARCHAR(4)  = 'A070'
...
------//프로시져 내부 변수정의//------
DECLARE @ERR_PGM        VARCHAR(50)   = 'SP_IF_Z50_IN';
...
DECLARE @ZONE_CD        VARCHAR(3) = 'Z50';
DECLARE @BEF_ZONE_CD    VARCHAR(3) = 'Z40';
            ...
            BEGIN
                ...
                IF  @@ROWCOUNT = 0
                    BEGIN
                        ...
                        RAISERROR(@P_PROC_MSG, 16, 1);
                    END;
                ----이동할 위치에 진행중인 BLOOM이 있으면 오류처리 --> 20180824 MJ 다수 BLOOM 이 있을수 있슴 주석처리. --> 다시 해제(Z50으로 이동할일 없슴)
