CREATE PROCEDURE dbo.SP_IF_QRCD
 @P_IF_SEQ    NUMERIC(12)
,@P_TC_CD     VARCHAR(4)  = 'A020'
...
------//프로시져 내부 변수정의//------
DECLARE @ERR_PGM        VARCHAR(50)   = 'SP_IF_QRCD';
...
DECLARE @BEF_ZONE_CD    VARCHAR(3) = 'Z01';
    ...
    BEGIN TRY
        ...
        BEGIN TRAN
            ...
            BEGIN
                ...
                WHERE  ZONE_CD       IN ('Z01','Z10')
                AND    PROG_STAT     = 'RUN'
                ORDER BY HEAT_NO, WORK_SEQ;
                IF  @@ROWCOUNT = 0
                    BEGIN
                        SET  @P_PROC_STAT  = 'ERR';
                        SET  @P_PROC_MSG   = 'ZONE에 작업할 정보가 없습니다. ';
                        RAISERROR(@P_PROC_MSG, 16, 1);
                    END;
                ----위치 이동 UPDATE
                UPDATE  TB_RL_TM_TRACKING
                SET     QR_SCAN_INFO      = @ZONE_CD
                       ,MODIFIER          = 'SYSTEM'
                       ,MOD_DDTT          = GETDATE()
