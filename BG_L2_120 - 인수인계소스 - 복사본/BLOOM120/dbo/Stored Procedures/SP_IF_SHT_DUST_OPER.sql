CREATE PROCEDURE [dbo].[SP_IF_SHT_DUST_OPER]
 @P_IF_SEQ    NUMERIC(12)
,@P_TC_CD     VARCHAR(4)  = 'H030'
...
------//프로시져 내부 변수정의//------
DECLARE @ERR_PGM        VARCHAR(50)   = 'SP_IF_SHT_DUST_OPER';
                ...
                IF  @@ROWCOUNT = 0
                    BEGIN
                        INSERT INTO TB_DUST_STAT_MGMT 
                               ( SHT_DUST_STAT, SHT_DUST_OFF_DDTT )
                        VALUES ( 'OFF', @REG_DDTT );
                    END;
                ELSE
                    --IF  @SHT_DUST_STAT = 'ON'
                        BEGIN
							INSERT INTO [dbo].[TB_SHT_DUST_INFO]
										([WORK_DDTT]
                                         ,[DUST_On_TR]
                                         ,[Bag_BEF_STATICP]
                                         ,[INH_TEMP]
									...
									VALUES
										...
										,CONVERT(INT,@DATA_20)			
										,CONVERT(INT,@DATA_21)			
										,CONVERT(NUMERIC(6,1),@DATA_22)	
										,CONVERT(NUMERIC(6,1),@DATA_23)	
										,CONVERT(NUMERIC(6,1),@DATA_24)	
										,CONVERT(NUMERIC(6,1),@DATA_25)	
										,CONVERT(NUMERIC(6,1),@DATA_26)	
										,CONVERT(NUMERIC(6,1),@DATA_27)	
										,CONVERT(NUMERIC(6,1),@DATA_28)	
										,CONVERT(NUMERIC(6,1),@DATA_29)	
										,'SYSTEM'						
										,GETDATE()						
										);								
									IF  @@ERROR = 0
